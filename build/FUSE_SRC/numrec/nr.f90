...
!  END INTERFACE
  ...
  INTERFACE beschb
    ...
    USE nrtype
    REAL(WP), INTENT(IN) :: x
    REAL(WP), INTENT(OUT) :: gam1,gam2,gampl,gammi
    END SUBROUTINE beschb_s
!BL
    SUBROUTINE beschb_v(x,gam1,gam2,gampl,gammi)
    USE nrtype
    REAL(WP), DIMENSION(:), INTENT(IN) :: x
    REAL(WP), DIMENSION(:), INTENT(OUT) :: gam1,gam2,gampl,gammi
...
!BL
  ...
  INTERFACE
    ...
    REAL(WP), DIMENSION(size(data)) :: convlv
    END FUNCTION convlv
  END INTERFACE
  INTERFACE
    FUNCTION correl(data1,data2)
    USE nrtype
    REAL(WP), DIMENSION(:), INTENT(IN) :: data1,data2
    REAL(WP), DIMENSION(size(data1)) :: correl
    END FUNCTION correl
  END INTERFACE
  INTERFACE
    SUBROUTINE cosft1(y)
...
!BL
  ...
  INTERFACE
    ...
    REAL(WP), DIMENSION(2*size(uc,1)-1,2*size(uc,1)-1) :: interp
    END FUNCTION interp
  END INTERFACE
  INTERFACE
    FUNCTION rank(indx)
  ...
  INTERFACE
    ...
    REAL(WP), DIMENSION(size(u,1),size(u,1)) :: lop
    END FUNCTION lop
  END INTERFACE
  INTERFACE
  ...
  INTERFACE
    ...
    USE nrtype
    REAL(WP), INTENT(OUT) :: ave,adev,sdev,var,skew,curt
    REAL(WP), DIMENSION(:), INTENT(IN) :: data
...
!BL
  ...
  INTERFACE
    ...
    USE nrtype
    REAL(WP), DIMENSION(:), INTENT(INOUT) :: cof
...
!BL
  ...
  INTERFACE
    ...
    USE nrtype
    REAL(WP), DIMENSION(:), INTENT(IN) :: data,d
...
!BL
  ...
  INTERFACE
    ...
    REAL(WP), DIMENSION(:), INTENT(OUT) :: cof
  ...
  INTERFACE ratval
    ...
    REAL(WP), INTENT(IN) :: x
    INTEGER(I4B), INTENT(IN) :: mm,kk
    REAL(WP), DIMENSION(mm+kk+1), INTENT(IN) :: cof
    REAL(WP) :: ratval_s
...
!BL
    ...
    INTEGER(I4B), INTENT(IN) :: mm,kk
    REAL(WP), DIMENSION(mm+kk+1), INTENT(IN) :: cof
    REAL(WP), DIMENSION(size(x)) :: ratval_v
...
!BL
  ...
  FUNCTION resid(u,rhs)
    ...
    REAL(WP), DIMENSION(size(u,1),size(u,1)) :: resid
    END FUNCTION resid
  END INTERFACE
  INTERFACE rf
    FUNCTION rf_s(x,y,z)
...
!BL
  ...
  INTERFACE
    ...
    REAL(WP), DIMENSION((size(uf,1)+1)/2,(size(uf,1)+1)/2) :: rstrct
    END FUNCTION rstrct
  END INTERFACE
  INTERFACE
  ...
  INTERFACE
    ...
    REAL(WP), DIMENSION(3,3), INTENT(IN) :: rhs
  ...
  INTERFACE
    ...
    USE nrtype
    REAL(WP), DIMENSION(:,:), INTENT(IN) :: a,b,c,d,e,f
    REAL(WP), DIMENSION(:,:), INTENT(INOUT) :: u
    REAL(WP), INTENT(IN) :: rjac
    END SUBROUTINE sor
  END INTERFACE
  INTERFACE
    SUBROUTINE sort(arr)
...
! and put it on the next one after that.
  ...
  INTERFACE
    FUNCTION vander(x,q)
    USE nrtype
    REAL(WP), DIMENSION(:), INTENT(IN) :: x,q
    REAL(WP), DIMENSION(size(x)) :: vander
    END FUNCTION vander
  END INTERFACE
  INTERFACE
