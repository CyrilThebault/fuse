MODULE FUSE_KGE_MODULE
  IMPLICIT NONE
  CONTAINS
  SUBROUTINE FUSE_KGE(XPAR,GRID_FLAG,NCID_FORC,KGE,OUTPUT_FLAG,IPSET,MPARAM_FLAG)
    ...
    ! Purpose:
    ! --------
    ! Calculate the KGE for single FUSE model and single parameter set
            ...
            IF(.NOT.elev_mask(iSpat1,iSpat2))THEN
              ...
              if(MFORCE%TEMP.gt.100.0) then; PRINT *, 'Temperature greater than 100 in input file'; stop; endif
               ...
               CASE(iopt_no_snowmod)
                  CALL QRAINERROR()
               CASE DEFAULT
                  message="f-fuse_kge/SMODL%iSNOWM must be either iopt_temp_index or iopt_no_snowmod"
                  RETURN
               END SELECT
    ...
    ! deallocate vectors
    DEALLOCATE(W_FLUX_3d); IF (IERR.NE.0) STOP ' problem deallocating W_FLUX_3d in fuse_kge '
    DEALLOCATE(STATE0,STATE1,STAT=IERR); IF (IERR.NE.0) STOP ' problem deallocating state vectors in fuse_kge '

  END SUBROUTINE FUSE_KGE
END MODULE FUSE_KGE_MODULE
