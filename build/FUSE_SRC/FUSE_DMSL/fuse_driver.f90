...
ELSE IF(fuse_mode == 'run_best')THEN  ! run FUSE with best (lowest KGE) parameter set from a previous SCE calibration
...
IF(fuse_mode == 'run_def')THEN ! run FUSE with default parameter values
  ...
  CALL FUSE_KGE(APAR,GRID_FLAG,NCID_FORC,KGE,OUTPUT_FLAG,NUMPSET)
...
ELSE IF(fuse_mode == 'run_pre')THEN ! run FUSE with pre-defined parameter values
  ...
  do IPSET = 1, NUMPSET
    ...
    CALL FUSE_KGE(APAR,GRID_FLAG,NCID_FORC,KGE,OUTPUT_FLAG,IPSET)
...
ELSE IF(fuse_mode == 'run_best')THEN ! run FUSE with best (highest KGE) parameter set from a previous SCE calibration
