...
REAL(SP)                               :: KGE             ! Kling-Gupta Efficiency
! ---------------------------------------------------------------------------------------
! (0) GET COMMAND-LINE ARGUMENTS
! ---------------------------------------------------------------------------------------
...
OPEN(21,FILE=TRIM(NC_FILE))
 DO   ! loop through output files
  ...
  PCOUNT=0          ! ensure the parameter counter is set to zero (incremented in fuse_kge)
  MOD_IX=MOD_IX + 1 ! increment the model index
  ! run zee model
  CALL FUSE_KGE(APAR,KGE,OUTPUT_FLAG,ERR=ERR,MESSAGE=MESSAGE)
  ! deallocate space
