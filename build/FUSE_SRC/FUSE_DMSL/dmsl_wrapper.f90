...
! Calls the SUBROUTINE fuse_kge.f90 to calculate the KGE for a given
...
use fuse_kge_module,only:fuse_kge        ! provide access to fuse_kge (run model)
...
if (present(objFuncM) .and. feas) then
 call fuse_kge(argInf,objFuncM,output_flag,mparam_flag)
endif
...
if (present(logp) .and. feas) then
 VAR=10._mrk**x(0)
 call fuse_kge(x(1:),kge,output_flag,mparam_flag)
