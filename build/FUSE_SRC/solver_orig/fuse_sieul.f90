...
IMPLICIT NONE
! input
REAL(WP), DIMENSION(:), INTENT(IN)            :: SINI        ! initial state vector
REAL(WP), DIMENSION(:), INTENT(IN)            :: DSDT0       ! initial state derivatives
REAL(WP), INTENT(IN)                          :: DT          ! time step
! internal
INTEGER(I4B)                                  :: ISTT        ! looping through states
REAL(WP), DIMENSION(SIZE(SINI))               :: STRY        ! trial state vector, used in FDJAC_ODE
REAL(WP), DIMENSION(SIZE(SINI),SIZE(SINI))    :: JAC_ODE     ! Jacobian of the ODE
REAL(WP), DIMENSION(SIZE(SINI),SIZE(SINI))    :: FJAC        ! Jacobian matrix
INTEGER(I4B), DIMENSION(SIZE(SINI))           :: INDX        ! Row permutations from partial pivoting (LUDCMP)
REAL(WP)                                      :: D           ! Denotes the number of row interchanges (LUDCMP)
REAL(WP), DIMENSION(SIZE(SINI))               :: DELS        ! Change in state variables
...
FJAC=-DT*JAC_ODE; CALL DIAGADD(FJAC,1._WP)       ! compute (I - DT dg/dS)
