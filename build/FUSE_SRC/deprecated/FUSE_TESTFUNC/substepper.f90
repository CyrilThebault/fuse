...
REAL(WP), DIMENSION(:), ALLOCATABLE    :: X_START     ! state vector at start of time interval
REAL(WP), DIMENSION(:), ALLOCATABLE    :: X0_DYDT     ! derivative at X0 (start)
REAL(WP), DIMENSION(:), ALLOCATABLE    :: XM_DYDT     ! derivative at XM (middle)
REAL(WP), DIMENSION(:), ALLOCATABLE    :: X1_DYDT     ! derivative at X1 (end)
REAL(WP), DIMENSION(:), ALLOCATABLE    :: XC_DYDT     ! corrected derivatives
REAL(WP), DIMENSION(:), ALLOCATABLE    :: X_END0      ! explicit one-step solution, end of time interval
REAL(WP), DIMENSION(:), ALLOCATABLE    :: X_END1      ! implicit one-step solution, end of time interval
REAL(WP), DIMENSION(:), ALLOCATABLE    :: X_MID2      ! implicit two-step solution, middle of time interval
REAL(WP), DIMENSION(:), ALLOCATABLE    :: X_END2      ! implicit two-step solution, end of time interval
REAL(WP), DIMENSION(:), ALLOCATABLE    :: EVEC        ! error estimate for each state
REAL(WP), DIMENSION(:), ALLOCATABLE    :: TVEC        ! error threshold for each state
REAL(WP)                               :: DT          ! time step used in explicit euler
LOGICAL(LGT)                           :: ERROR_FLAG  ! .TRUE. if extrapolation error
LOGICAL(LGT)                           :: NEW_DERIVS  ! .TRUE. if need to calculate new derivatives
REAL(WP)                               :: STEPSAVE    ! save the time step (HSTATE%STEP altered for two-step solution)
...
REAL(WP)                               :: FTIM        ! fraction of model time interval to advance states
...
SUBSTEPS: DO  ! continuous (recursive) loop over sub-steps
 ...
 SELECT CASE(SOLUTION_METHOD)
  ...
  CASE (EXPLICIT_EULER)
   ...
   ! -------------------------------------------------------------------------------------
   DT = HSTATE%STEP/2.0_WP                   ! define the time step
  ...
  CASE (IMPLICIT_EULER)
   ...
   SELECT CASE (INITIAL_NEWTON) 
    ...
    CASE (EXPLICIT_MID);  X_END1 = X_START + X0_DYDT*HSTATE%STEP/2.0_WP ! estimate at mid-point
   ...
   SELECT CASE(TRUNCATION_ERROR)
    ...
    CASE (STEP_HALVING)
     ...
     HSTATE%STEP = HSTATE%STEP/2._wp              ! new HSTATE%STEP for use in FUNCV
    ...
    CASE (EMBEDDED_ERR)
     ...
     DT     = HSTATE%STEP
     X_END2 = X_START + 0.5_WP*(X0_DYDT+X1_DYDT)*DT
...
CONTAINS
 ...
 SELECT CASE(SMALL_ENDSTEP)
  ...
  CASE(LOOK_AHEAD)  ! the look-ahead method of Shampine (1994)
   ...
   ELSE
    IF (ETIME + STEP*2._WP .GE. DELTIM) THEN
     REVISE_STEP = (DELTIM - ETIME)/2._WP
    ELSE
     REVISE_STEP = STEP
    ENDIF
   ENDIF
  ...
  CASE(STEP_ABSORB) ! the step-absorption method
   ...
   ELSE
    ...
    IF (ETIME + STEP + T_MGN .GE. DELTIM) THEN
     ...
     STEP_INCREASE = .TRUE. 
    ELSE
     IF (ETIME + STEP + T_MGN*2._WP .GE. DELTIM) THEN
      REVISE_STEP   = STEP + T_MGN*(1._WP - (DELTIM-(ETIME+STEP))/T_MGN)
      STEP_INCREASE = .TRUE.
     ELSE
      REVISE_STEP = STEP
     ENDIF
