...
IMPLICIT NONE
REAL(WP), DIMENSION(1)             :: STATE0      ! initial state
REAL(WP), DIMENSION(1)             :: STATE1      ! final state
...
DO ITYP=0,1
 ...
 DO ITNC=1,4
  ...
  STATE0         =     1._WP   ! state at the start of the time step
  STATE1         = -9999._WP   ! state at the end of the time step
  MS_MIN%WATR_1  = 1.E-10_WP   ! minimum values of model states (shared in MODULE test_modvar)
  MS_MAX%WATR_1  = 1.E+00_WP   ! maximum values of model states (shared in MODULE test_modvar)
  FSTATE%WATR_1  =     1._WP   ! initial value of model states (shared in MODULE test_modvar)
  DT_SUB         =     1._WP   ! length of sub-step
  DT_FULL        =     1._WP   ! length of full step
  print *, '*********************************************************************************'
  print *, '*********************************************************************************'
  ...
  DO ITIME=1,NTIME
   ...
   W_FLUX%DRAINAGE = 0._WP 
   W_FLUX%CHECKTIM = 0._WP 
...
MAX_TSTEP              = 1440._wp/60._wp/24._wp ! maximum time step length (minutes --> days)
