...
INTEGER(I4B), PARAMETER    :: EXPLICIT_EULER=0, EXPLICIT_HEUN=1, IMPLICIT_EULER=2, &
                              IMPLICIT_HEUN=3, SEMI_IMPLICIT=4
...
! 3. Method used to estimate the initial conditions for the Newton scheme
INTEGER(I4B), PARAMETER    :: STATE_OLD=0, EXPLICIT_MID=1, EXPLICIT_FULL=2
INTEGER(I4B)               :: INITIAL_NEWTON 
! 4. Jacobian re-evaluation strategy
INTEGER(I4B), PARAMETER    :: FULLYVARIABLE=0, CONST_SUBSTEP=1, CONSTFULLSTEP=2, PERIOD_FREEZE=3, &
                              SMALL_F_RATIO=4
INTEGER(I4B)               :: JAC_RECOMPUTE
REAL(SP), ALLOCATABLE      :: fjacDCMP(:,:), fjacCOPY(:,:), fjacINDX(:) ! (temporary arrays)
! 5. Method used to trap/fix errors in Newton
...
REAL(SP)                   :: THRESH_FRZE    ! Threshold for freezing the Jacobian 
