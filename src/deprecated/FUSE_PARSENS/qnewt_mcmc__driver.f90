...
USE fuse_fileManager,only:FORCINGINFO,OUTPUT_PATH         ! directory for data files
! data modules
...
REAL(WP), DIMENSION(:), ALLOCATABLE    :: OF_VALS         ! objective functioni values
...
real(mrk),dimension(:),allocatable     :: sample0,sdevDiag0
! Define files
...
! auxiliaries
character(50)::string(0:9)
integer(I4B)::parSetHow
...
if(read_arg)then
  ...
  MBASIN_ID="mahurangi"
  FMODEL_ID="070"
  NSOLUTION="3"  ! 0=explicit Euler, 1=explicit Heun, 2=implicit Euler, 3=imp Heun, 4=semi-implicit Euler
  FADAPTIVE="1"  ! 0=fixed-step, 1=adaptive
  TRUNC_ABS="1.e-2"
  TRUNC_REL="1.e-2"
  NUM_MULTI="2"
  SOBOLSEED="1"
...
!MAX_TSTEP = 0.1_wp                       ! forces dense substeps (eg, when checking solutions)
...
allocate(sample0(0:numpar),sdevDiag0(0:numpar))
!DK-note 31 March 2012: I do not recall why sample0 and sdevDiag0 are (0:) rather than (1:)
...
FNAME_PREFIX = TRIM(OUTPUT_PATH)//'DMSL_'//TRIM(MBASIN_ID)//'__'//TRIM(SMODL%MNAME)//'__'//&
               ...
               TRIM(TRUNC_ABS)//'__'//TRIM(TRUNC_REL)
! define NetCDF files (filename shared in MODULE model_defn) 
FNAME_NETCDF = TRIM(FNAME_PREFIX)//'__mcmc.nc' 
CALL DEF_PARAMS(ONEMOD)  ! define model parameters (initial CREATE)
IF (OUTPUT_FLAG) CALL DEF_OUTPUT(NTIM)    ! define model time series (REDEF)
CALL DEF_SSTATS()        ! define summary statistics (REDEF)
! get MCMC samples
parSetHow=1
selectcase(parSetHow)
case default
  sample0(0)  = log10(FOPT**2)  ! variance
  sample0(1:) = XOPT(1:)
case(1) ! 9/10/2009: checking MCMC method
  sample0=(/-1.09935261642738E-001_mrk,&
             1.01300120353699E+000_mrk,&
             9.49999988079071E-001_mrk,&
             2.24372955322266E+002_mrk,&
             5.00000000000000E+003_mrk,&
             1.48710086941719E-002_mrk,&
             1.20471649169922E+001_mrk,&
             2.00000000000000E+001_mrk,&
             8.33424106240273E-002_mrk,&
             8.67173850536346E-001_mrk /)
  sdevDiag0=-666.6_mrk
  string=(/"BFBC24B7A16EC99A",&
           "3FF03540C0000000","3FEE666660000000","406C0BEF40000000",&
           "40B3880000000000","3F8E74B100000000","4028182600000000",&
           "4034000000000000","3FB555EDA0000000","3FEBBFE360000000"/)
  do i=0,9
    read(string(i),'(Z16.16)',iostat=err)sdevDiag0(i)
  enddo
!   sample0=sdevDiag0
case(2) ! 9/10/2009: checking against FUSE visualdriver
  sample0(0)=0._mrk ! log10(VAR), assume VAR=1
  do ipar=1,numpar
   call getpar_str(lparam(ipar)%parname,param_meta)
   sample0(ipar)=param_meta%pardef
  end do
endselect
sdevDiag0(0)    = 0.1_mrk*max(abs(sample0(0)),1._mrk)
sdevDiag0(1:)   = 0.1_mrk*max(abs(sample0(1:)),xsc)
!DK 31/03/2012: since we switched from var to sdev, this now means 10% sdev rather than 10% var,
!               but this is just initialisation of MCMC, so its ok.
CALL MCMC_WRAPPER(sample0=sample0,sdevDiag0=sdevDiag0,ierr=err,message=message)
if(err/=0)then
  write(*,*)trim(message)
  stop
endif
...
!FNAME_PRODKT = TRIM(FNAME_PREFIX)//'__testin.sam'
...
IF (ERR.NE.0) THEN; PRINT *, ERR, ' PROBLEM OPENING FILE '; STOP; ENDIF
 DO ! continuous do loop with exit clause
  ...
  MSTATS%JUMP_TAKEN = 0._WP; IF (JUMP_FLAG) MSTATS%JUMP_TAKEN = 1._WP ! (convert flag to real) 
...
DEALLOCATE(X0I,XLO,XHI,XSC,URAND,XOPT,sample0,sdevDiag0)
STOP
END PROGRAM QNEWT_MCMC__DRIVER
! --------------------------------------------------------------------------------------
