...
! ---------------------------------------------------------------------------------------
CHARACTER(LEN=12)                      :: DATSUBSET='            ' ! data subset used (PERIOD1; PERIOD2; ALLDATA)
...
INTEGER(I4B),PARAMETER                 :: NGRID=10001        ! number of samples across a single parameter dimension
! Looping variables
...
REAL(WP)                               :: T1,T2           ! CPU time
! ---------------------------------------------------------------------------------------
! (0) READ COMMAND LINE ARGUMENTS
! ---------------------------------------------------------------------------------------
! read command-line arguments
CALL GETARG(1,DATSUBSET)  ! data subset used (PERIOD1; PERIOD2; ALLDATA)
...
CALL GETARG(6,PAR_NAME1)  ! name of the 1st parameter in the grid
CALL GETARG(7,PAR_NAME2)  ! name of the 2nd parameter in the grid
! check command-line arguments
IF (LEN_TRIM(DATSUBSET).EQ.0) STOP '1st command-line argument is missing (DATSUBSET)'
...
IF (LEN_TRIM(PAR_NAME1).EQ.0) STOP '6th command-line argument is missing (PAR_NAME1)'
IF (LEN_TRIM(PAR_NAME2).EQ.0) STOP '7th command-line argument is missing (PAR_NAME2)'
...
! ---------------------------------------------------------------------------------------
! Define data file (shared in ddirectory)
FORCINGINFO='forcinginfo.'//TRIM(DATSUBSET)//'.txt'
...
DO IPAR=1,NUMPAR
 ...
 XDF(IPAR) = PARAM_META%PARDEF
 !PRINT *, LPARAM(IPAR), PARAM_META%PARLOW, PARAM_META%PARUPP
END DO
IPARSET = 0
...
DO
 ...
 WRITE(*,'(20(A12,1X))') LPARAM(1:NUMPAR); WRITE(*,'(20(F12.6,1X))') XDF
 ! increment counter
 IPARSET = IPARSET + 1
 ...
 FNAME_NETCDF = TRIM(OUTPUT_PATH)//TRIM(DATSUBSET)//'__'//TRIM(SMODL%MNAME)//'__'//CPARSET//'__'//&
                TRIM(NSOLUTION)//'-'//TRIM(FADAPTIVE)//'__'//&
                TRIM(TRUNC_ABS)//'-'//TRIM(TRUNC_REL)//'__'//&
                TRIM(PAR_NAME1)//'-'//TRIM(PAR_NAME2)//'__pargrid.nc'
...
DEALLOCATE(fjacCOPY,fjacDCMP,fjacINDX)
IF (IERR.NE.0) STOP ' problem deallocating space for jacabian copies '
STOP
